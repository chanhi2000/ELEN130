...
__(c)__ Using MATLAB’s freqz, find the system frequency response and plot the
magnitude. Verify that the frequency response has notches at two distinct frequencies. What is the maximum gain of the system? (Just take the maximum of the plotted magnitude.)
...
## 7.
Specify the cutoff frequencies of low pass filters needed to create the following desired filters by adding and subtracting multiple low pass filters. Using MATLAB, find the coefficients for the desired filter and plot both the magnitude and the log magnitude (that is $$20\log_{10}$$) — __Note__: you can protect against the problem of taking the log of $$0$$ by making sure the minimum magnitude cannot be lower than a very small value. Use $$M=15$$ (length = 31). Note, you can use the filter designs from the last homework.
...
__(a)__ Find an expression for $$W_1(z)$$, its $$z$$-transform. It may be useful to write this as a summation and find a simplifcation for the summation.
